...
import { GameState } from "./types/GameService.types";
...
io.on("connection", (socket) => {
  ...
  socket.on("game.grid.selected", (data) => {
    ...
    // Here calcul score
    games[gameIndex].gameState.player1Score = GameService.score.getCurrentScore(
      "player:1",
      games[gameIndex].gameState
    );
    games[gameIndex].player1Socket.emit(
      "game.score",
      GameService.send.forPlayer.gameScore(
        "player:1",
        games[gameIndex].gameState
      )
    );
