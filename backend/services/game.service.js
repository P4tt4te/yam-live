...
const GameService = {
        ...
        forPlayer: {
            viewGameState: (playerKey, game) => {
                return {
                    inQueue: false,
                    inGame: true,
                    idPlayer: playerKey === "player:1"
                        ? game.player1Socket.id
                        : game.player2Socket.id,
                    idOpponent: playerKey === "player:1"
                        ? game.player2Socket.id
                        : game.player1Socket.id,
                };
            },
    ...
    dices: {
        ...
        lockEveryDice: (dicesToLock) => {
            const lockedDices = dicesToLock.map((dice) => (Object.assign(Object.assign({}, dice), { locked: true })));
            return lockedDices;
        },
    },
    choices: {
    ...
    grid: {
        resetcanBeCheckedCells: (grid) => {
            const updatedGrid = grid.map((row) => row.map((cell) => {
                return Object.assign(Object.assign({}, cell), { canBeChecked: false });
            }));
            return updatedGrid;
        },
        updateGridAfterSelectingChoice: (idSelectedChoice, grid) => {
            const updatedGrid = grid.map((row) => row.map((cell) => {
                if (cell.id === idSelectedChoice && cell.owner === null) {
                    return Object.assign(Object.assign({}, cell), { canBeChecked: true });
                }
                else {
                    return cell;
                }
        ...
        selectCell: (idCell, rowIndex, cellIndex, currentTurn, grid) => {
            const updatedGrid = grid.map((row, rowIndexParsing) => row.map((cell, cellIndexParsing) => {
                if (cell.id === idCell &&
                    ...
                    return Object.assign(Object.assign({}, cell), { owner: currentTurn });
                }
                else {
                    return cell;
                }
